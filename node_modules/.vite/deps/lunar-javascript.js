...
//#region node_modules/lunar-javascript/lunar.js
var require_lunar = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(root, factory) {
		if (typeof define === "function" && define.amd) define(factory);
		else if (typeof module != "undefined" && module.exports) module.exports = factory();
		else {
			var o = factory();
			for (var i in o) root[i] = o[i];
		}
	})(exports, function() {
		var Solar = (function() {
			...
			var _fromYmdHms = function(y, m, d, hour, minute, second) {
				...
				var os = second;
				y *= 1;
				if (isNaN(y)) throw new Error("wrong solar year " + oy);
				m *= 1;
				if (isNaN(m)) throw new Error("wrong solar month " + om);
				d *= 1;
				if (isNaN(d)) throw new Error("wrong solar day " + od);
				hour *= 1;
				if (isNaN(hour)) throw new Error("wrong hour " + oh);
				minute *= 1;
				if (isNaN(minute)) throw new Error("wrong minute " + oi);
				second *= 1;
				if (isNaN(second)) throw new Error("wrong second " + os);
				if (1582 === y && 10 === m) {
					if (d > 4 && d < 15) throw new Error("wrong solar year " + y + " month " + m + " day " + d);
				}
				...
				return {
					...
					subtractMinute: function(solar) {
						...
						var m = this._p.hour * 60 + this._p.minute - (solar.getHour() * 60 + solar.getMinute());
						if (m < 0) {
							m += 1440;
							days--;
						}
					...
					isAfter: function(solar) {
						if (this._p.year > solar.getYear()) return true;
						if (this._p.year < solar.getYear()) return false;
						if (this._p.month > solar.getMonth()) return true;
						if (this._p.month < solar.getMonth()) return false;
						if (this._p.day > solar.getDay()) return true;
						if (this._p.day < solar.getDay()) return false;
						if (this._p.hour > solar.getHour()) return true;
						if (this._p.hour < solar.getHour()) return false;
						if (this._p.minute > solar.getMinute()) return true;
						if (this._p.minute < solar.getMinute()) return false;
					...
					*/
					...
					toFullString: function() {
						var s = this.toYmdHms();
						if (this.isLeapYear()) s += " 闰年";
						s += " 星期" + this.getWeekInChinese();
						var festivals = this.getFestivals();
						for (var i = 0, j = festivals.length; i < j; i++) s += " (" + festivals[i] + ")";
						s += " " + this.getXingZuo() + "座";
						return s;
					},
					nextYear: function(years) {
						var oy = years;
						years *= 1;
						if (isNaN(years)) throw new Error("wrong years " + oy);
						...
						if (1582 === y && 10 === m) {
							...
							if (d > 28) {
								if (!SolarUtil.isLeapYear(y)) d = 28;
							}
						}
					...
					nextMonth: function(months) {
						var om = months;
						months *= 1;
						if (isNaN(months)) throw new Error("wrong months " + om);
					...
					nextDay: function(days) {
						var od = days;
						days *= 1;
						if (isNaN(days)) throw new Error("wrong days " + od);
						var y = this._p.year;
						var m = this._p.month;
						var d = this._p.day;
						if (1582 === y && 10 === m) {
						...
						if (days !== 0) {
							...
							while (rest > 0) {
								...
								if (!holiday) {
									...
									if (0 === week || 6 === week) work = false;
								} else work = holiday.isWork();
								if (work) rest -= 1;
							}
						}
						return solar;
					...
					next: function(days, onlyWorkday) {
						if (onlyWorkday) return this.nextWorkday(days);
						return this.nextDay(days);
					},
					nextHour: function(hours) {
						var oh = hours;
						hours *= 1;
						if (isNaN(hours)) throw new Error("wrong hours " + oh);
					...
					getJulianDay: function() {
						...
						var n = 0;
						var g = false;
						if (y * 372 + m * 31 + Math.floor(d) >= 588829) g = true;
						if (m <= 2) {
							m += 12;
							y--;
						}
					...
					getSalaryRate: function() {
						...
						if (holiday) {
							if (!holiday.isWork()) return 2;
						} else {
							var week = this.getWeek();
							if (week === 6 || week === 0) return 2;
						}
						return 1;
					}
				};
			...
			var _fromBaZi = function(yearGanZhi, monthGanZhi, dayGanZhi, timeGanZhi, sect, baseYear) {
				...
				if (m < 0) m += 12;
				if (((LunarUtil.index(yearGanZhi.substring(0, 1), LunarUtil.GAN, -1) + 1) * 2 + m) % 10 !== LunarUtil.index(monthGanZhi.substring(0, 1), LunarUtil.GAN, -1)) return l;
				...
				while (y <= endYear) {
					if (y >= startYear) {
						...
						var solarTime = jieQiLunar.getJieQiTable()[jieQiList[4 + m]];
						if (solarTime.getYear() >= baseYear) {
							...
							if (d < 0) d += 60;
							if (d > 0) solarTime = solarTime.next(d);
							for (var i = 0, j = hours.length; i < j; i++) {
								...
								if (d === 30) solar = solar.nextHour(-1);
								var lunar = solar.getLunar();
								var dgz = 2 === sect ? lunar.getDayInGanZhiExact2() : lunar.getDayInGanZhiExact();
								if (lunar.getYearInGanZhiExact() === yearGanZhi && lunar.getMonthInGanZhiExact() === monthGanZhi && dgz === dayGanZhi && lunar.getTimeInGanZhi() === timeGanZhi) l.push(solar);
							}
						}
					}
					y += 60;
		...
		var Lunar = (function() {
			...
			var _computeYear = function(o, solar, year) {
				...
				if (liChun.getYear() !== solarYear) liChun = o["jieQi"]["LI_CHUN"];
			...
			var _computeMonth = function(o, solar) {
				...
				for (i = 0; i < size; i += 2) {
					...
					var ymd = solar.toYmd();
					if (ymd >= (null == start ? ymd : start.toYmd()) && ymd < end.toYmd()) break;
					start = end;
					index++;
				}
				...
				for (i = 0; i < size; i += 2) {
					...
					if (time >= (null == start ? time : start.toYmdHms()) && time < end.toYmdHms()) break;
					start = end;
					index++;
				}
			...
			var _fromYmdHms = function(lunarYear, lunarMonth, lunarDay, hour, minute, second) {
				...
				var os = second;
				lunarYear *= 1;
				if (isNaN(lunarYear)) throw new Error("wrong lunar year " + oy);
				lunarMonth *= 1;
				if (isNaN(lunarMonth)) throw new Error("wrong lunar month " + om);
				lunarDay *= 1;
				if (isNaN(lunarDay)) throw new Error("wrong lunar day " + od);
				hour *= 1;
				if (isNaN(hour)) throw new Error("wrong hour " + oh);
				minute *= 1;
				if (isNaN(minute)) throw new Error("wrong minute " + oi);
				...
				if (null == m) throw new Error("wrong lunar year " + lunarYear + " month " + lunarMonth);
				if (lunarDay < 1) throw new Error("lunar day must bigger than 0");
				var days = m.getDayCount();
				if (lunarDay > days) throw new Error("only " + days + " days in lunar year " + lunarYear + " month " + lunarMonth);
				...
				if (noon.getYear() !== lunarYear) y = LunarYear.fromYear(noon.getYear());
			...
			var _new = function(year, month, day, hour, minute, second, solar, ly) {
				...
				return {
					...
					getYearInChinese: function() {
						...
						for (var i = 0, j = y.length; i < j; i++) s += LunarUtil.NUMBER[y.charCodeAt(i) - zero];
						return s;
					},
					...
					getDayPositionTaiSui: function(sect) {
						...
						].join(",").indexOf(dayInGanZhi) > -1) p = I18n.getMessage("bg.zhen");
						else if ([
						...
						].join(",").indexOf(dayInGanZhi) > -1) p = I18n.getMessage("bg.li");
						else if ([
						...
						].join(",").indexOf(dayInGanZhi) > -1) p = I18n.getMessage("ps.center");
						else if ([
						...
						].join(",").indexOf(dayInGanZhi) > -1) p = I18n.getMessage("bg.dui");
						else if ([
						...
						].join(",").indexOf(dayInGanZhi) > -1) p = I18n.getMessage("bg.kan");
						else p = LunarUtil.POSITION_TAI_SUI_YEAR[yearZhiIndex];
						return p;
					},
					...
					getYearPositionTaiSui: function(sect) {
						...
						switch (sect) {
							...
							case 3:
								yearZhiIndex = this._p.yearZhiIndexExact;
								break;
							default: yearZhiIndex = this._p.yearZhiIndexByLiChun;
						}
					...
					getDayChongShengXiao: function() {
						...
						for (var i = 0, j = LunarUtil.ZHI.length; i < j; i++) if (LunarUtil.ZHI[i] === chong) return LunarUtil.SHENGXIAO[i];
						return "";
					},
					...
					_convertJieQi: function(name) {
						var jq = name;
						if ("DONG_ZHI" === jq) jq = I18n.getMessage("jq.dongZhi");
						else if ("DA_HAN" === jq) jq = I18n.getMessage("jq.daHan");
						else if ("XIAO_HAN" === jq) jq = I18n.getMessage("jq.xiaoHan");
						else if ("LI_CHUN" === jq) jq = I18n.getMessage("jq.liChun");
						else if ("DA_XUE" === jq) jq = I18n.getMessage("jq.daXue");
						else if ("YU_SHUI" === jq) jq = I18n.getMessage("jq.yuShui");
					...
					getJie: function() {
						for (var i = 0, j = LunarUtil.JIE_QI_IN_USE.length; i < j; i += 2) {
							...
							if (d.getYear() === this._p.solar.getYear() && d.getMonth() === this._p.solar.getMonth() && d.getDay() === this._p.solar.getDay()) return this._convertJieQi(key);
						}
						return "";
					},
					getQi: function() {
					...
					getOtherFestivals: function() {
						...
						if (this._p.solar.toYmd() === this._getJieQiSolar(I18n.getMessage("jq.qingMing")).next(-1).toYmd()) l.push("寒食节");
					...
					_getYearNineStar: function(yearInGanZhi) {
						...
						var yearOffset = indexExact - (LunarUtil.getJiaZiIndex(this.getYearInGanZhi()) + 1);
						if (yearOffset > 1) yearOffset -= 60;
						else if (yearOffset < -1) yearOffset += 60;
						var offset = (62 + Math.floor((this._p.year + yearOffset + 2696) / 60) % 3 * 3 - indexExact) % 9;
					...
					getYearNineStar: function(sect) {
						...
						switch (sect) {
							...
							case 3:
								...
								break;
							default: yearInGanZhi = this.getYearInGanZhiByLiChun();
						}
					...
					getMonthNineStar: function(sect) {
						...
						if (monthZhiIndex < LunarUtil.BASE_MONTH_ZHI_INDEX) n -= 3;
					...
					getDayNineStar: function() {
						...
						var solarShunBai2;
						var solarNiZi;
						if (dongZhiIndex > 29) solarShunBai = dongZhi.next(60 - dongZhiIndex);
						else solarShunBai = dongZhi.next(-dongZhiIndex);
						var solarShunBaiYmd = solarShunBai.toYmd();
						if (dongZhiIndex2 > 29) solarShunBai2 = dongZhi2.next(60 - dongZhiIndex2);
						else solarShunBai2 = dongZhi2.next(-dongZhiIndex2);
						var solarShunBaiYmd2 = solarShunBai2.toYmd();
						if (xiaZhiIndex > 29) solarNiZi = xiaZhi.next(60 - xiaZhiIndex);
						else solarNiZi = xiaZhi.next(-xiaZhiIndex);
						var solarNiZiYmd = solarNiZi.toYmd();
						var offset = 0;
						if (solarYmd >= solarShunBaiYmd && solarYmd < solarNiZiYmd) offset = this._p.solar.subtract(solarShunBai) % 9;
						else if (solarYmd >= solarNiZiYmd && solarYmd < solarShunBaiYmd2) offset = 8 - this._p.solar.subtract(solarNiZi) % 9;
						else if (solarYmd >= solarShunBaiYmd2) offset = this._p.solar.subtract(solarShunBai2) % 9;
						else if (solarYmd < solarShunBaiYmd) offset = (8 + solarShunBai.subtract(this._p.solar)) % 9;
						return NineStar.fromIndex(offset);
					},
					getTimeNineStar: function() {
						var solarYmd = this._p.solar.toYmd();
						var asc = false;
						if (solarYmd >= this._getJieQiSolar(I18n.getMessage("jq.dongZhi")).toYmd() && solarYmd < this._getJieQiSolar(I18n.getMessage("jq.xiaZhi")).toYmd() || solarYmd >= this._getJieQiSolar("DONG_ZHI").toYmd()) asc = true;
						var start = (asc ? [
							0,
							3,
							6
					...
					getNextJie: function(wholeDay) {
						var conditions = [];
						for (var i = 0, j = LunarUtil.JIE_QI_IN_USE.length / 2; i < j; i++) conditions.push(LunarUtil.JIE_QI_IN_USE[i * 2]);
					...
					getNextQi: function(wholeDay) {
						var conditions = [];
						for (var i = 0, j = LunarUtil.JIE_QI_IN_USE.length / 2; i < j; i++) conditions.push(LunarUtil.JIE_QI_IN_USE[i * 2 + 1]);
						return this._getNearJieQi(true, conditions, wholeDay);
					},
					...
					_buildJieQi: function(name, solar) {
						var jie = false;
						var qi = false;
						for (var i = 0, j = LunarUtil.JIE_QI.length; i < j; i++) if (LunarUtil.JIE_QI[i] === name) {
							if (i % 2 === 0) qi = true;
							else jie = true;
							break;
						}
					...
					_getNearJieQi: function(forward, conditions, wholeDay) {
						...
						var filters = {};
						var filter = false;
						if (null != conditions) for (var i = 0, j = conditions.length; i < j; i++) {
					...
					getCurrentJieQi: function() {
						for (var key in this._p.jieQi) {
							var d = this._getJieQiSolar(key);
							if (d.getYear() === this._p.solar.getYear() && d.getMonth() === this._p.solar.getMonth() && d.getDay() === this._p.solar.getDay()) return this._buildJieQi(this._convertJieQi(key), d);
						}
						return null;
					},
					...
					getEightChar: function() {
						if (!this._p.eightChar) this._p.eightChar = EightChar.fromLunar(this);
					...
					toFullString: function() {
						...
						var i;
						var j;
						for (i = 0, j = festivals.length; i < j; i++) s += " (" + festivals[i] + ")";
					...
					getShuJiu: function() {
						...
						if (currentDay.isBefore(startDay) || !currentDay.isBefore(endDay)) return null;
					...
					getFu: function() {
						...
						var days = currentDay.subtract(startDay);
						if (days < 10) return this._buildNameAndIndex("初伏", days + 1);
						startDay = startDay.next(10);
						days = currentDay.subtract(startDay);
						if (days < 10) return this._buildNameAndIndex("中伏", days + 1);
						startDay = startDay.next(10);
						var liQiuDay = Solar.fromYmd(liQiu.getYear(), liQiu.getMonth(), liQiu.getDay());
						days = currentDay.subtract(startDay);
						if (liQiuDay.isAfter(startDay)) {
							if (days < 10) return this._buildNameAndIndex("中伏", days + 11);
							startDay = startDay.next(10);
							days = currentDay.subtract(startDay);
						}
					...
					getTimes: function() {
						...
						for (var i = 0; i < 12; i++) l.push(LunarTime.fromYmdHms(this._p.year, this._p.month, this._p.day, (i + 1) * 2 - 1, 0, 0));
						return l;
					},
					getFoto: function() {
				...
				return {
					...
					*/
					next: function(weeks, separateMonth) {
						var ow = weeks;
						weeks *= 1;
						if (isNaN(weeks)) throw new Error("wrong weeks " + ow);
						var start = this._p.start;
						if (0 === weeks) return _fromYmd(this._p.year, this._p.month, this._p.day, start);
						...
						if (separateMonth) {
							...
							while (0 !== n) {
								...
								if (month !== weekMonth) {
									...
									else if (SolarUtil.getWeeksOfMonth(week.getYear(), week.getMonth(), start) === index) {
					...
					*/
					getFirstDayInMonth: function() {
						...
						for (var i = 0; i < days.length; i++) if (this._p.month === days[i].getMonth()) {
							index = i;
							break;
						}
						return days[index];
					...
					*/
					getDays: function() {
						...
						var l = [];
						l.push(firstDay);
						for (var i = 1; i < 7; i++) l.push(firstDay.next(i));
						return l;
					},
					/**
					* 获取本周的阳历日期列表（仅限当月）
					...
					*/
					getDaysInMonth: function() {
						...
						for (var i = 0; i < days.length; i++) {
							var day = days[i];
							if (this._p.month !== day.getMonth()) continue;
							l.push(day);
						}
						return l;
					},
			...
			var _fromYm = function(y, m) {
				...
				return {
					...
					getDays: function() {
						...
						for (var i = 1; i < days; i++) l.push(d.next(i));
						return l;
					},
					getWeeks: function(start) {
						...
						while (true) {
							...
							var firstDay = week.getFirstDay();
							if (firstDay.getYear() > this._p.year || firstDay.getMonth() > this._p.month) break;
						}
						return l;
					},
					toString: function() {
				...
				return {
					...
					*/
					next: function(seasons) {
						var os = seasons;
						seasons *= 1;
						if (isNaN(seasons)) throw new Error("wrong seasons " + os);
					...
					*/
					getMonths: function() {
						...
						for (var i = 0; i < 3; i++) l.push(SolarMonth.fromYm(this._p.year, 3 * index + i + 1));
						return l;
					},
					toString: function() {
				...
				return {
					...
					*/
					next: function(halfYears) {
						var oh = halfYears;
						halfYears *= 1;
						if (isNaN(halfYears)) throw new Error("wong halfYears " + oh);
					...
					*/
					getMonths: function() {
						var l = [];
						var index = this.getIndex() - 1;
						for (var i = 0; i < 6; i++) l.push(SolarMonth.fromYm(this._p.year, 6 * index + i + 1));
						return l;
					},
					toString: function() {
						return this.getYear() + "." + this.getIndex();
			...
			var _fromYear = function(y) {
				...
				return {
					...
					getMonths: function() {
						...
						l.push(m);
						for (var i = 1; i < 12; i++) l.push(m.next(i));
						return l;
					},
					toString: function() {
						return this.getYear() + "";
		...
		var LunarYear = (function() {
			...
			var _inLeap = function(arr, n) {
				for (var i = 0, j = arr.length; i < j; i++) if (arr[i] === n) return true;
				return false;
			};
				...
				return {
					...
					getDayCount: function() {
						...
						for (var i = 0, j = this._p.months.length; i < j; i++) {
							...
							if (m.getYear() === this._p.year) n += m.getDayCount();
						}
						return n;
					},
					getMonthsInYear: function() {
						...
						for (var i = 0, j = this._p.months.length; i < j; i++) {
							var m = this._p.months[i];
							if (m.getYear() === this._p.year) l.push(m);
						}
						return l;
					},
					getMonths: function() {
					...
					getMonth: function(lunarMonth) {
						var om = lunarMonth;
						lunarMonth *= 1;
						if (isNaN(lunarMonth)) throw new Error("wrong lunarMonth " + om);
						for (var i = 0, j = this._p.months.length; i < j; i++) {
							var m = this._p.months[i];
							if (m.getYear() === this._p.year && m.getMonth() === lunarMonth) return m;
						}
						return null;
					},
					...
					getNineStar: function() {
						...
						var offset = (62 + Math.floor((this._p.year + 2696) / 60) % 3 * 3 - index) % 9;
						if (0 === offset) offset = 9;
						return NineStar.fromIndex(offset - 1);
					},
					getPositionXi: function() {
					...
					_compute: function() {
						...
						for (i = 0, j = LunarUtil.JIE_QI_IN_USE.length; i < j; i++) {
							if (i === 0) jd = ShouXingUtil.qiAccurate2(jq[0] - 15.2184);
							else if (i <= 26) jd = ShouXingUtil.qiAccurate2(jq[i - 1]);
							else jd = ShouXingUtil.qiAccurate2(jq[25] + 15.2184 * (i - 26));
						...
						var leapIndex = 16;
						if (_inLeap(_LEAP_11, currentYear)) leapIndex = 13;
						else if (_inLeap(_LEAP_12, currentYear)) leapIndex = 14;
						else if (hs[13] <= jq[24]) {
							i = 1;
							while (hs[i + 1] > jq[2 * i] && i < 13) i++;
							leapIndex = i;
						}
						...
						for (i = 0; i < 15; i++) {
							...
							var v2 = months[i];
							var mc = _YMC[v2 % 12];
							if (1724360 <= dm && dm < 1729794) mc = _YMC[(v2 + 1) % 12];
							else if (1807724 <= dm && dm < 1808699) mc = _YMC[(v2 + 1) % 12];
							else if (dm === 1729794 || dm === 1808699) mc = 12;
							if (fm === -1) {
								fm = mc;
								index = mc;
							}
							...
							fm = mc;
							if (i === leapIndex) mc = -mc;
							else if (dm === 1729794 || dm === 1808699) mc = -11;
			...
			return { fromYear: function(lunarYear) {
			...
			var _new = function(lunarYear, lunarMonth, dayCount, firstJulianDay, index) {
				return {
					...
					getPositionTaiSui: function() {
						...
						switch (Math.abs(this._p.month)) {
							...
							case 12:
								p = "巽";
								break;
							default: p = LunarUtil.POSITION_GAN[Solar.fromJulianDay(this.getFirstJulianDay()).getLunar().getMonthGanIndex()];
						}
						return p;
					},
					getPositionTaiSuiDesc: function() {
					...
					next: function(n) {
						var on = n;
						n *= 1;
						if (isNaN(n)) throw new Error("wrong days " + on);
						if (0 === n) return LunarMonth.fromYm(this._p.year, this._p.month);
						else {
							var rest = Math.abs(n);
							var ny = this._p.year;
							var iy = ny;
		...
		var ShouXingUtil = (function() {
			...
			return {
				...
				eLon: function(t, n) {
					...
					for (var i = 0; i < 6; i++, tn *= t) {
						...
						if (n < 0) m = n2;
							...
							m = Math.floor(3 * n * n0 / m0 + .5 + n1);
							if (i !== 0) m += 3;
							if (m > n2) m = n2;
						}
						c = 0;
						for (var j = n1; j < m; j += 3) c += this.XL0[j] * Math.cos(this.XL0[j + 1] + t * this.XL0[j + 2]);
						v += c * tn;
					}
					v /= this.XL0[0];
					var t2 = t * t;
					v += (-.0728 - 2.7702 * t - 1.1019 * t2 - .0996 * t2 * t) / this.SECOND_PER_RAD;
					return v;
				},
				mLon: function(t, n) {
					var ob = this.XL1;
					...
					var t5 = t4 * t;
					var tx = t - 10;
					v += (3.81034409 + 8399.684730072 * t - 3319e-8 * t2 + 3.11e-8 * t3 - 2033e-13 * t4) * this.SECOND_PER_RAD;
					v += 5028.792262 * t + 1.1124406 * t2 + 7699e-8 * t3 - 23479e-9 * t4 - 1.78e-8 * t5;
					if (tx > 0) v += -.866 + 1.43 * tx + .054 * tx * tx;
					t2 /= 1e4;
					t3 /= 1e8;
					t4 /= 1e8;
					n *= 6;
				...
				gxcSunLon: function(t) {
					var t2 = t * t;
					var v = -.043126 + 628.301955 * t - 2732e-9 * t2;
					return -20.49552 * (1 + (.016708634 - 42037e-9 * t - 1.267e-7 * t2) * Math.cos(v)) / this.SECOND_PER_RAD;
				},
				ev: function(t) {
					var f = 628.307585 * t;
					return 628.332 + 21 * Math.sin(1.527 + f) + .44 * Math.sin(1.48 + f * 2) + .129 * Math.sin(5.82 + f) * t + 55e-5 * Math.sin(4.21 + f) * t * t;
				},
				saLon: function(t, n) {
				...
				dtCalc: function(y) {
					...
					var i;
					for (i = 0; i < size; i += 5) if (y < this.DT_AT[i + 5]) break;
				...
				mv: function(t) {
					var v = 8399.71 - 914 * Math.sin(.7848 + 8328.691425 * t + 1523e-7 * t * t);
				...
				saLonT2: function(w) {
					...
					t -= (5297e-9 * t * t + .0334166 * Math.cos(4.669257 + 628.307585 * t) + 2061e-7 * Math.cos(2.67823 + 628.307585 * t) * t) / v;
				...
				msaLonT2: function(w) {
					...
					t = (w + 1.08472) / v;
					var t2 = t * t;
					t -= (-3309e-8 * t2 + .10976 * Math.cos(.784758 + 8328.6914246 * t + 152292e-9 * t2) + .02224 * Math.cos(.1874 + 7214.0628654 * t - 21848e-8 * t2) - .03342 * Math.cos(4.669257 + 628.307585 * t)) / v;
					t2 = t * t;
					l = this.mLon(t, 20) - (4.8950632 + 628.3319653318 * t + 5297e-9 * t2 + .0334166 * Math.cos(4.669257 + 628.307585 * t) + 2061e-7 * Math.cos(2.67823 + 628.307585 * t) * t + 349e-6 * Math.cos(4.6261 + 1256.61517 * t) - 20.5 / this.SECOND_PER_RAD);
					v = 7771.38 - 914 * Math.sin(.7848 + 8328.691425 * t + 1523e-7 * t2) - 179 * Math.sin(2.543 + 15542.7543 * t) - 160 * Math.sin(.1874 + 7214.0629 * t);
					t += (w - l) / v;
					return t;
				},
				qiHigh: function(w) {
					...
					var v = (t + .5) % 1 * this.SECOND_PER_DAY;
					if (v < 1200 || v > this.SECOND_PER_DAY - 1200) t = this.saLonT(w) * 36525 - this.dtT(t) + this.ONE_THIRD;
					return t;
				},
				shuoHigh: function(w) {
					...
					t = t - this.dtT(t) + this.ONE_THIRD;
					var v = (t + .5) % 1 * this.SECOND_PER_DAY;
					if (v < 1800 || v > this.SECOND_PER_DAY - 1800) t = this.msaLonT(w) * 36525 - this.dtT(t) + this.ONE_THIRD;
					return t;
				},
				qiLow: function(w) {
					var v = 628.3319653318;
				...
				shuoLow: function(w) {
					var v = 7771.37714500204;
					var t = (w + 1.08472) / v;
					t -= (-331e-7 * t * t + .10976 * Math.cos(.785 + 8328.6914 * t) + .02224 * Math.cos(.187 + 7214.0629 * t) - .03342 * Math.cos(4.669 + 628.3076 * t)) / v + (32 * (t + 1.8) * (t + 1.8) - 20) / this.SECOND_PER_DAY / 36525;
					return t * 36525 + this.ONE_THIRD;
				},
				calcShuo: function(jd) {
					...
					if (jd < f1 || jd >= f3) d = Math.floor(this.shuoHigh(Math.floor((jd + pc - 2451551) / 29.5306) * Math.PI * 2) + .5);
					else if (jd >= f1 && jd < f2) {
						for (i = 0; i < size; i += 2) if (jd + pc < this.SHUO_KB[i + 2]) break;
						d = this.SHUO_KB[i] + this.SHUO_KB[i + 1] * Math.floor((jd + pc - this.SHUO_KB[i]) / this.SHUO_KB[i + 1]);
						d = Math.floor(d + .5);
						if (d === 1683460) d++;
						d -= Solar.J2000;
					} else if (jd >= f2 && jd < f3) {
						d = Math.floor(this.shuoLow(Math.floor((jd + pc - 2451551) / 29.5306) * Math.PI * 2) + .5);
				...
				calcQi: function(jd) {
					...
					if (jd < f1 || jd >= f3) d = Math.floor(this.qiHigh(Math.floor((jd + pc - 2451259) / 365.2422 * 24) * Math.PI / 12) + .5);
					else if (jd >= f1 && jd < f2) {
						for (i = 0; i < size; i += 2) if (jd + pc < this.QI_KB[i + 2]) break;
						d = this.QI_KB[i] + this.QI_KB[i + 1] * Math.floor((jd + pc - this.QI_KB[i]) / this.QI_KB[i + 1]);
						d = Math.floor(d + .5);
						if (d === 1683460) d++;
						d -= Solar.J2000;
					} else if (jd >= f2 && jd < f3) {
						d = Math.floor(this.qiLow(Math.floor((jd + pc - 2451259) / 365.2422 * 24) * Math.PI / 12) + .5);
		...
		var SolarUtil = (function() {
			return {
				...
				getDaysOfMonth: function(year, month) {
					...
					var om = month;
					year *= 1;
					if (isNaN(year)) throw new Error("wrong solar year " + oy);
					month *= 1;
					if (isNaN(month)) throw new Error("wrong solar month " + om);
					if (1582 === year && 10 === month) return 21;
					var m = month - 1;
					var d = this.DAYS_OF_MONTH[m];
					if (m === 1 && this.isLeapYear(year)) d++;
					return d;
				},
				...
				getDaysInYear: function(year, month, day) {
					...
					if (isNaN(month)) throw new Error("wrong solar month " + om);
					day *= 1;
					if (isNaN(day)) throw new Error("wrong solar day " + od);
					var days = 0;
					for (var i = 1; i < month; i++) days += this.getDaysOfMonth(year, i);
					var d = day;
					if (1582 === year && 10 === month) {
						if (day >= 15) d -= 10;
						else if (day > 4) throw new Error("wrong solar year " + year + " month " + month + " day " + day);
					}
					days += d;
					return days;
				},
				getDaysBetween: function(ay, am, ad, by, bm, bd) {
					...
					var obd = bd;
					ay *= 1;
					if (isNaN(ay)) throw new Error("wrong solar year " + oay);
					am *= 1;
					if (isNaN(am)) throw new Error("wrong solar month " + oam);
					ad *= 1;
					if (isNaN(ad)) throw new Error("wrong solar day " + oad);
					by *= 1;
					if (isNaN(by)) throw new Error("wrong solar year " + oby);
					bm *= 1;
					if (isNaN(bm)) throw new Error("wrong solar month " + obm);
					bd *= 1;
					if (isNaN(bd)) throw new Error("wrong solar day " + obd);
					var n;
					var days;
					var i;
					if (ay === by) n = this.getDaysInYear(by, bm, bd) - this.getDaysInYear(ay, am, ad);
					else if (ay > by) {
						days = this.getDaysOfYear(by) - this.getDaysInYear(by, bm, bd);
						for (i = by + 1; i < ay; i++) days += this.getDaysOfYear(i);
						days += this.getDaysInYear(ay, am, ad);
						n = -days;
					} else {
						days = this.getDaysOfYear(ay) - this.getDaysInYear(ay, am, ad);
						for (i = ay + 1; i < by; i++) days += this.getDaysOfYear(i);
		...
		var LunarUtil = (function() {
			return {
				...
				getTimeZhiIndex: function(hm) {
					if (!hm) return 0;
					if (hm.length > 5) hm = hm.substring(0, 5);
					var x = 1;
					for (var i = 1; i < 22; i += 2) {
						if (hm >= (i < 10 ? "0" : "") + i + ":00" && hm <= (i + 1 < 10 ? "0" : "") + (i + 1) + ":59") return x;
						x++;
					}
					return 0;
				},
				...
				getDayYi: function(monthGanZhi, dayGanZhi) {
					...
					while (index > -1) {
						...
						var left = right;
						if (left.indexOf("=") > -1) left = left.substring(0, left.indexOf("=") - 2);
						var matched = false;
						var months = left.substring(0, left.indexOf(":"));
						var i;
						var j;
						for (i = 0, j = months.length; i < j; i += 2) if (months.substring(i, i + 2) === month) {
							matched = true;
							break;
						}
						if (matched) {
							...
							for (i = 0, j = ys.length; i < j; i += 2) l.push(this.YI_JI[parseInt(ys.substring(i, i + 2), 16)]);
							break;
						}
				...
				getDayJi: function(monthGanZhi, dayGanZhi) {
					...
					while (index > -1) {
						...
						if (matched) {
							...
							for (i = 0, j = js.length; i < j; i += 2) l.push(this.YI_JI[parseInt(js.substring(i, i + 2), 16)]);
							break;
						}
						index = right.indexOf(day + "=");
					}
				...
				getDayJiShen: function(monthZhiIndex, dayGanZhi) {
					...
					if (matcher) {
						...
						for (var i = 0, j = data.length; i < j; i += 2) {
							...
							if (n < 60) l.push(this.SHEN_SHA[n + 1]);
						}
					}
					if (l.length < 1) l.push(this.SHEN_SHA[0]);
					return l;
				...
				getTimeYi: function(dayGanZhi, timeGanZhi) {
					...
					if (index > -1) {
						...
						if (left.indexOf("=") > -1) left = left.substring(0, left.indexOf("=") - 4);
						var ys = left.substring(0, left.indexOf(","));
						for (var i = 0, j = ys.length; i < j; i += 2) l.push(this.YI_JI[parseInt(ys.substring(i, i + 2), 16)]);
					}
					if (l.length < 1) l.push(this.SHEN_SHA[0]);
					return l;
				},
				getTimeJi: function(dayGanZhi, timeGanZhi) {
					...
					if (index > -1) {
						...
						if (left.indexOf("=") > -1) left = left.substring(0, left.indexOf("=") - 4);
						var js = left.substring(left.indexOf(",") + 1);
						for (var i = 0, j = js.length; i < j; i += 2) l.push(this.YI_JI[parseInt(js.substring(i, i + 2), 16)]);
					}
					if (l.length < 1) l.push(this.SHEN_SHA[0]);
					return l;
				},
				...
				index: function(name, names, offset) {
					for (var i = 0, j = names.length; i < j; i++) if (names[i] === name) return i + offset;
					return -1;
				},
				find: function(s, arr) {
		...
		var HolidayUtil = (function(_NAMES) {
			...
			var _DATA_IN_USE = "";
			...
			var _buildHolidayForward = function(s) {
				...
				return _buildHoliday(day, name, s.charCodeAt(9) === _ZERO, s.substring(10, 18));
			};
			var _buildHolidayBackward = function(s) {
				var size = s.length;
				var day = s.substring(size - 18, size - 10);
				var name = _NAMES_IN_USE[s.charCodeAt(size - 10) - _ZERO];
				return _buildHoliday(day, name, s.charCodeAt(size - 9) === _ZERO, s.substring(size - 8));
			};
			var _findForward = function(key) {
				...
				if (n > 0) right = right.substring(n);
				while (0 !== right.indexOf(key) && right.length >= _SIZE) right = right.substring(_SIZE);
				return right;
			};
			var _findBackward = function(key) {
				...
				var n = size % _SIZE;
				if (n > 0) left = left.substring(0, size - n);
				size = left.length;
			...
			var _fixData = function(data) {
				...
				while (data.length >= _SIZE) {
					...
					if (!holiday) {
						if (!remove) append.push(segment);
					} else {
						var nameIndex = -1;
						for (var i = 0, j = _NAMES_IN_USE.length; i < j; i++) if (_NAMES_IN_USE[i] === holiday.getName()) {
							nameIndex = i;
							break;
						}
						if (nameIndex > -1) {
				...
				if (append.length > 0) _DATA_IN_USE += append.join("");
			};
		...
		var EightChar = (function() {
			var _fromLunar = function(lunar) {
				return {
					...
					getYearShiShenZhi: function() {
						...
						var l = [];
						for (var i = 0, j = hideGan.length; i < j; i++) l.push(LunarUtil.SHI_SHEN[dayGan + hideGan[i]]);
						return l;
					},
					_getDiShi: function(zhiIndex) {
						var index = LunarUtil.CHANG_SHENG_OFFSET[this.getDayGan()] + (this.getDayGanIndex() % 2 === 0 ? zhiIndex : -zhiIndex);
					...
					getMingGong: function() {
						var offset = LunarUtil.index(this.getMonthZhi(), LunarUtil.MONTH_ZHI, 0) + LunarUtil.index(this.getTimeZhi(), LunarUtil.MONTH_ZHI, 0);
						offset = (offset >= 14 ? 26 : 14) - offset;
						var ganIndex = (this._p.lunar.getYearGanIndexExact() + 1) * 2 + offset;
						while (ganIndex > 10) ganIndex -= 10;
					...
					getShenGong: function() {
						var offset = LunarUtil.index(this.getMonthZhi(), LunarUtil.MONTH_ZHI, 0) + LunarUtil.index(this.getTimeZhi(), LunarUtil.ZHI, 0);
					...
					getYun: function(gender, sect) {
						...
						var start = (function() {
							...
							if (2 === sect) {
								...
								var hourDiff = (end.getHour() === 23 ? 11 : LunarUtil.getTimeZhiIndex(end.toYmdHms().substring(11, 16))) - (start.getHour() === 23 ? 11 : LunarUtil.getTimeZhiIndex(start.toYmdHms().substring(11, 16)));
							...
							return {
								...
								getGanZhi: function() {
									var offset = [
										...
										8,
										0
									][(LunarUtil.find(this._p.liuNian.getGanZhi(), LunarUtil.GAN).index - 1) % 5];
									return LunarUtil.GAN[(this._p.index + offset) % 10 + 1] + LunarUtil.ZHI[(this._p.index + LunarUtil.BASE_MONTH_ZHI_INDEX) % 12 + 1];
								},
								getXun: function() {
						...
						var buildLiuNian = function(daYun, index) {
							return {
								...
								getGanZhi: function() {
									...
									if (this._p.daYun.getIndex() > 0) offset += this._p.daYun.getStartAge() - 1;
						...
						var buildXiaoYun = function(daYun, index, forward) {
							return {
								...
								getGanZhi: function() {
									...
									if (this._p.daYun.getIndex() > 0) add += this._p.daYun.getStartAge() - 1;
			...
			return { fromLunar: function(lunar) {
		...
		var LunarTime = (function() {
			var _fromYmdHms = function(lunarYear, lunarMonth, lunarDay, hour, minute, second) {
				...
				return {
					...
					getNineStar: function() {
						...
						var asc = false;
						if (solarYmd >= jieQi[I18n.getMessage("jq.dongZhi")].toYmd() && solarYmd < jieQi[I18n.getMessage("jq.xiaZhi")].toYmd()) asc = true;
						var start = (asc ? [
							0,
							3,
							6
					...
					getMinHm: function() {
						...
						if (hour < 1) return "00:00";
						else if (hour > 22) return "23:00";
						if (hour % 2 === 0) hour -= 1;
						return (hour < 10 ? "0" : "") + hour + ":00";
					},
					getMaxHm: function() {
						var hour = this._p.lunar.getHour();
						if (hour < 1) return "00:59";
						else if (hour > 22) return "23:59";
						if (hour % 2 !== 0) hour += 1;
			...
			return { fromYmdHms: function(lunarYear, lunarMonth, lunarDay, hour, minute, second) {
		...
		var FotoUtil = (function() {
			...
			var _f = function(name, result, everyMonth, remark) {
				return {
					...
					toFullString: function() {
						var l = [this._p.name];
						if (this._p.result) l.push(this._p.result);
			...
			var _fromLunar = function(lunar) {
				return {
					...
					isDayYangGong: function() {
						...
						for (var i = 0, j = l.length; i < j; i++) if ("杨公忌" === l[i].getName()) return true;
						return false;
					},
					...
					isDayZhaiSix: function() {
						var d = this.getDay();
						if (8 === d || 14 === d || 15 === d || 23 === d || 29 === d || 30 === d) return true;
						else if (28 === d) {
							if (30 !== LunarMonth.fromYm(this._p.lunar.getYear(), this.getMonth()).getDayCount()) return true;
						}
						return false;
					},
					...
					isDayZhaiGuanYin: function() {
						...
						for (var i = 0, j = FotoUtil.DAY_ZHAI_GUAN_YIN.length; i < j; i++) if (k === FotoUtil.DAY_ZHAI_GUAN_YIN[i]) return true;
						return false;
					},
					getXiu: function() {
					...
					toFullString: function() {
						...
						for (var i = 0, j = fs.length; i < j; i++) s += " (" + fs[i] + ")";
						return s;
					}
				};
			};
		...
		var TaoFestival = (function() {
			...
			return { create: function(name, remark) {
			...
			var _fromLunar = function(lunar) {
				return {
					...
					getFestivals: function() {
						...
						if (I18n.getMessage("jq.dongZhi") === jq) l.push(TaoFestival.create("元始天尊圣诞"));
						else if (I18n.getMessage("jq.xiaZhi") === jq) l.push(TaoFestival.create("灵宝天尊圣诞"));
						var f = TaoUtil.BA_JIE[jq];
						if (f) l.push(TaoFestival.create(f));
					...
					_isDayIn: function(days) {
						...
						for (var i = 0, j = days.length; i < j; i++) if (md === days[i]) return true;
						return false;
					},
		...
		var I18n = (function() {
			...
			var _updateArray = function(c) {
				...
				for (var k in v) {
					var arr = v[k];
					for (var i = 0, j = arr.length; i < j; i++) o[k][i] = arr[i].replace(/{(.[^}]*)}/g, function($0, $1) {
			...
			var _updateArrayDictionary = function(c) {
				...
				for (var k in v) {
					...
					for (var key in dict) {
						...
						var arr = dict[key];
						for (var i = 0, j = arr.length; i < j; i++) arr[i] = arr[i].replace(/{(.[^}]*)}/g, function($0, $1) {
							return _getMessage($1);
						});
						o[k][x] = arr;
					}
			...
			var _getMessage = function(key) {
				...
				if (void 0 === s) s = _messages[_defaultLang][key];
				if (void 0 === s) s = key;
				return s;
			};
			var _initArray = function(c) {
				...
				for (var k in v) {
					v[k].length = 0;
					var arr = o[k];
					for (var i = 0, j = arr.length; i < j; i++) v[k].push(arr[i]);
				}
			};
			var _initDictionary = function(c, type) {
				...
				for (var k in v) {
					var dict = o[k];
					for (var key in dict) v[k][key] = dict[key];
				}
			};
			var _init = function() {
				if (_inited) return;
...
//#endregion
//#region node_modules/lunar-javascript/index.js
var require_lunar_javascript = /* @__PURE__ */ __commonJSMin(((exports, module) => {
